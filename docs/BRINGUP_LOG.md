...
- AXIS ignores SIGTERM, so it has to be stopped with `axis-remote --quit`. (That fell out of `lcnc-ctl` when it became a service wrapper; it's now the unit's `ExecStop`. See the stale-lock entry below.)
...
- **Gotcha:** `lcnc-ctl restart` let systemd SIGKILL AXIS after the 30 s stop timeout. That left `/tmp/linuxcnc.lock`, and the next start waited on an invisible Tk dialog. After clearing the lock, task still hung in `exec_state 7` and ignored MDI and abort. A clean stop (`axis-remote --quit`) and a fresh start fixed it. Fixed for good in the next entry.

## 2026-09-24: Stop hang and stale lock fixed

- **Symptom:** `lcnc-ctl restart` hit `TimeoutStopSec=30`. systemd SIGKILLed linuxcnc, rtapi_app, milltask and axis, and `/tmp/linuxcnc.lock` was left behind. The next start blocked on LinuxCNC's "still running, restart?" Tk dialog (`wish8.6`) on the headless display :1, so `lcnc-ctl start` hung. Manual recovery: `systemctl stop`, `rm /tmp/linuxcnc.lock`, `halrun -U` as arduino, then start.
